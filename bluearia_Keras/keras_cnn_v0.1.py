...
def generate_model(train = False, ep=50, patience=5):
    ...
    '''Settings explanation : 128 is the number of available frequencies (number of mels in spectrogram 
    creation) 2500 is time division (len(s_test.S[1]) = 20) 1 correspond to the number of channels, here 
    we only work with grey scale, so 1 channel is enough''' 
    ...
    dropout1 = keras.layers.Dropout(0.25)(pool1) # TEST 
    ...
    gAvgPool = keras.layers.GlobalAveragePooling2D()(dropout3) 
    ...
    output_layer = keras.layers.Dense(
        units=7, 
        activation='softmax'
        )(dropout4) # Units = number of classes 
...
generate_model(train=True, ep=50, patience=6)